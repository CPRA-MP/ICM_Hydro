...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_TMP_R09(npr, ioutfile, input_file, rday, ndtr, nlatr_ori, nlatr, latFlowLoc, latFlowTp, latFlowXsec)
  use precision
!  use params
  use mod_arrays_TMP_R09

  use sed_properties_TMP_R09
  use spline_func
 ...
 ! integer :: nx, nt, np
  integer :: LUN, ioerr
    ...
    open(unit = LUN, file=trim(input_file)//'/input/tmp.inp', &
   status='old', action = 'read', iostat = ioerr)

      if (ioerr /= 0) then
      write(*, *) 'ERROR reading the input file: tmp.inp'
      stop
      endif

!read(LUN, '(a)') RefDate   ! Reference date, Format: MM-DD-YYYY_HH:MM:SS or MM-DD-YYYYTHH:MM:SS (not used at the moment)
    read(LUN, *) tmp_init   ! Reference date, Format: MM-DD-YYYY_HH:MM:SS or MM-DD-YYYYTHH:MM:SS (not used at the moment)
...
!	pause 1
	...
	! read temperature parameters <-Hu
	read(LUN, *) Total_area
	! This file contains U10
					   						
    read(LUN, *) reach_U10
	reach_U10=trim(input_file)//reach_U10								
	! This file contains Tback
    read(LUN, *) reach_Tbk
	reach_Tbk=trim(input_file)//reach_Tbk
  close (LUN)
!!!!!!!!!!!!!!!!!!!!


  write(*, *) 'Tmp_init    = ', tmp_init
  write(*, *) 'Tstart     = ', Tstart
  write(*, *) 'Tstop      = ', Tstop
  write(*, *) 'Dt         = ', Dt
  write(*, *) 'DtUser     = ', DtUser
...
!!!!! UNIT = 13: contains the 2D array data for the cross section area (time, space) = (t, x)
  ...
  ! read temperature files <-Hu
  ! U10
	LUN = 999
  open(unit = LUN, file = trim(reach_U10))
       read(LUN,*) !Skip comment line
      read(LUN, *) ntU10
  ...
  ! Tback
	LUN = 999
  open(unit = LUN, file = trim(reach_Tbk))
       read(LUN,*) !Skip comment line
      read(LUN, *) ntbk
...
! We do this outside the main time loop because we need some BC values
! before entering the time loop
allocate(Bcoef(ntU10))
allocate(Ccoef(ntU10))
allocate(Dcoef(ntU10))
allocate(tmp1D(nt))


write(*, *) 'Applying spline interpolation in time on U10'

																								
			
  Bcoef = 0.0_fp; Ccoef = 0.0_fp; Dcoef = 0.0_fp

  call spline(ntU10, TimeU10, U10, Bcoef, Ccoef, Dcoef)

  do n = 1, nt
    fval = ispline(time(n), ntU10, TimeU10, U10, Bcoef, Ccoef, Dcoef)
    tmp1D(n) = fval
  end do
  
	  

deallocate(U10); allocate(U10(nt))
U10 = tmp1D
...
!--------------------
print*,'U10=',U10(1), U10(int(nt/2)),U10(nt)
!pause 33
!--------------------<-Hu
! Interpolate Tbk
! We do this outside the main time loop because we need some BC values
...
write(*, *) 'Applying spline interpolation in time on Tbk'
			   
  Bcoef = 0.0_fp; Ccoef = 0.0_fp; Dcoef = 0.0_fp

  call spline(ntbk, TimeTbk, Tbk, Bcoef, Ccoef, Dcoef)

  do n = 1, nt
    fval = ispline(time(n), ntbk, TimeTbk, Tbk, Bcoef, Ccoef, Dcoef)
    tmp1D(n) = fval
  end do
  
deallocate(Tbk); allocate(Tbk(nt))
Tbk = tmp1D
...
deallocate(TimeTbk)
!--------------------
print*,'Tbk=',Tbk(1), Tbk(int(nt/2)),Tbk(nt)
!pause 44
...
open(ioutfile,file=trim(out_dir)//'Tmp_output.dat')
write(ioutfile,30)time(1)/60, ((CN(i,j),i=1,nx),j=1,np)
30  format(<nx*np+1>f12.2)

end subroutine init_TMP_R09


subroutine cal_TMP_R09(n, npr, ioutfile, nlatt, inp_depth, inp_area, inp_flow, out_tmp, Q_lat_from_ICM, TMP_lat_from_ICM, TMP_upstream_from_ICM, TMP_terminal_from_ICM)
  use precision
!  use params
  use mod_arrays_TMP_R09
  ...
  !integer :: LUN, ioerr
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R09_TMP Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
  ...
  if (n /= nt) then
    ...
    do k = 1, np
      do i = 2, nx - 1
        ...
        SrcTerm = 0.0
        if( useSRC ) then
	      SrcTerm = srctemp(U10(n),Tbk(n),CN(i, k),width(i))
	!	  print*,U10(n),Tbk(n),CN(i, k),width(i)
	!	  print*, 'SrCTerm=', SrcTerm
...
!          endif
		...
		do j=1, Nlat
			...
			if (i >= ilat .AND. i < (ilat+jlat))then
				...
				!coupling from ICM
				if (Q_lat_from_ICM(j) .gt. 0. .and. TMP_lat_from_ICM(j) .ge. 0. .and. klat .eq. 3)then
					SrcTerm = SrcTerm+Q_lat_from_ICM(j)/real(jlat)*(TMP_lat_from_ICM(j)-CN(i,k))/(abs(flow(i))+Q_lat_from_ICM(j)/real(jlat))
				endif

				if (klat.eq.2) then
					print*, 'Rating curves for lateral temperature input have not been supported yet.'
					stop
				endif
 ...
 !       CN1(i, k) = aa * CN(i+1, k) + bb * CN(i, k) + cc * CN(i-1, k) + (dt / arni) * SrcTerm
	  ...
	  if(TMP_upstream_from_ICM .ge. 0. )CN1(1, k)=TMP_upstream_from_ICM   !coupling from ICM
	  if (flow(i) < 0.)then
		CN1(1, k)=CN1(2, k)
	  endif
	 ...
	 if (bc_option == 1) then
        ...
        SrcTerm = 0.0
        if( useSRC ) then
			SrcTerm = srctemp(U10(n),Tbk(n),CN(nx, k),width(nx))											   
!          if (D50(k) < 0.000062) then
...
!          endif
		...
		if(TMP_terminal_from_ICM .ge. 0. )CN1(nx, k)=TMP_terminal_from_ICM  !coupling from ICM
	endif
