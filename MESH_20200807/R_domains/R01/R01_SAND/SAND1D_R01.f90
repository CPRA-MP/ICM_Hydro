...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_SAND_R01(ioutfile, input_file)
  use precision
!  use params
  use mod_arrays_SAND_R01

  use sed_properties_SAND_R01
  use spline_func
...
end subroutine init_SAND_R01


subroutine cal_SAND_R01(n, npr, ioutfile, nlatt, inp_depth, inp_area, inp_flow, out_sand, Q_lat_from_ICM, SAND_lat_from_ICM, SAND_upstream_from_ICM, SAND_terminal_from_ICM)
  use precision
!  use params
  use mod_arrays_SAND_R01
  ...
  !integer :: LUN, ioerr
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R01_SAND Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
      ...
      do i = 2, nx - 1
		...
		do j=1, Nlat
			...
			if (i >= ilat .AND. i < (ilat+jlat))then
				...
				!coupling from ICM
				if (Q_lat_from_ICM(j) .gt. 0. .and. SAND_lat_from_ICM(j) .ge. 0.)then
