...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_TMP_R01(ioutfile, input_file)
  use precision
!  use params
 ...
 ! integer :: nx, nt, np
  integer :: LUN, ioerr
    ...
    open(unit = LUN, file=trim(input_file), &
   status='old', action = 'read', iostat = ioerr)

      if (ioerr /= 0) then
      write(*, *) 'ERROR reading the input file: sed1d_20200130.inp'
      stop
      endif
