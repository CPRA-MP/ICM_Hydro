...
module params_TMP_R00
  ...
  real(fp) :: tmp_init, Tstart, Tstop, Dt
  ...
  integer, allocatable :: Idlat(:), Nsclat(:)    ! Lateral flow id and # of CSs
  ...
  real(fp) :: Cf, alphaSED, SEDn, SEDcalib
  ...
  ! For Temperature module <-Hu
  real(fp) :: Total_area  !Total surface water area in m2
...
! For lateral flow with the concentration is specified
  ...
  ! For wind at 10m <- Hu
  integer  :: ntU10
  real(fp), allocatable :: U10(:), TimeU10(:)
  
  ! For background Temperature
  integer  :: ntbk
  real(fp), allocatable :: Tbk(:), TimeTbk(:)
 
  character(len=256) :: reach_U10, reach_Tbk


end module params_TMP_R00