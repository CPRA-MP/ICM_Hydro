...
!    close(51)
!	close(61)
!end subroutine end_R06

subroutine init_R06(npr, ifile, input_file, rday, ndt, nlat_ori, nlat, latFlowLoc, latFlowTp, latFlowXsec, outa, outb, outc, outd, wl_lat, Q_terminal)
    use constants_module_R06
    use arrays_module_R06
    use arrays_section_module_R06
    use var_module_R06
    use matrix_module_R06
    use sgate_module_R06
...
!	endif
    ...
    do i=1,ncomp
        read(999, *) x, sk(i)
        call readXsection_R06(i,(1.0/sk(i)),timesDepth)
...
end subroutine init_R06



subroutine cal_R06(n, npr, ifile, nlat, outa, outb, outc, outd, wl_lat, WL_terminal_from_ICM, Q_upstream_from_ICM, Q_lat_from_ICM, Q_terminal)
        ...
        do i=1,noLatFlow
			...
			elseif (latFlowType(i) .eq. 3) then
				if(Q_lat_from_ICM(i)<-99990.)then
					print*,'R06 ERROR: Q_lat_from_ICM at #',i
					stop
				endif
...
!-------------------------------------
		...
		if(mod(n+1,24*saveFrequency) .eq. 0 .or. (n.eq.0))write(*,*)'R06: Nstep =', n+1, 'Days = ' &
