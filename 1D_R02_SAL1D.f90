...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_SAL_R02(npr, ioutfile, input_file, rday, ndtr, nlatr_ori, nlatr, latFlowLoc, latFlowTp, latFlowXsec)
  use precision
!  use params
  use mod_arrays_SAL_R02

  use sed_properties_SAL_R02
  use spline_func
 ...
 ! integer :: nx, nt, np
  integer :: LUN, ioerr
    ...
    open(unit = LUN, file=trim(input_file)//'/input/sal.inp', &
   status='old', action = 'read', iostat = ioerr)

      if (ioerr /= 0) then
      write(*, *) 'ERROR reading the input file: sal.inp'
      stop
      endif
    !read(LUN, '(a)') RefDate   ! Reference date, Format: MM-DD-YYYY_HH:MM:SS or MM-DD-YYYYTHH:MM:SS (not used at the moment)
    read(LUN, *) sal_init   ! Reference date, Format: MM-DD-YYYY_HH:MM:SS or MM-DD-YYYYTHH:MM:SS (not used at the moment)
...
open(ioutfile,file=trim(out_dir)//'SalCon_output.dat')
write(ioutfile,30)time(1)/60, ((CN(i,j),i=1,nx),j=1,np)
30  format(<nx*np+1>f12.2)

end subroutine init_SAL_R02


subroutine cal_SAL_R02(n, npr, ioutfile, nlatt, inp_depth, inp_area, inp_flow, out_sal, Q_lat_from_ICM, SAL_lat_from_ICM, SAL_upstream_from_ICM, SAL_terminal_from_ICM)
  use precision
!  use params
  use mod_arrays_SAL_R02
  ...
  !integer :: LUN, ioerr
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R02_SAL Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
...
30  format(<nx*np+1>f12.2)    
 ...
 !       CN1(i, k) = aa * CN(i+1, k) + bb * CN(i, k) + cc * CN(i-1, k) + (dt / arni) * SrcTerm
	  ...
	  if(SAL_upstream_from_ICM .ge. 0. )CN1(1, k)=SAL_upstream_from_ICM   !coupling from ICM
	  if (flow(i) < 0.)then
		CN1(1, k)=CN1(2, k)
	  endif
	 ...
	 if (bc_option == 1) then
      ...
      elseif (bc_option == 3) then
		...
		if(SAL_terminal_from_ICM .ge. 0. )CN1(nx, k)=SAL_terminal_from_ICM  !coupling from ICM
	endif
