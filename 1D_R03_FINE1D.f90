...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_FINE_R03(npr, ioutfile, input_file, rday, ndtr, nlatr_ori, nlatr, latFlowLoc, latFlowTp, latFlowXsec)
    use precision
    !use params
    use mod_arrays_FINE_R03
    
    use sed_properties_FINE_R03
    use spline_func
...
end subroutine init_FINE_R03


subroutine cal_FINE_R03(n, npr, ioutfile, nlatt, inp_depth, inp_area, inp_flow, out_fine, Q_lat_from_ICM, FINE_lat_from_ICM, FINE_upstream_from_ICM, FINE_terminal_from_ICM)
    use precision
    !use params
    use mod_arrays_FINE_R03
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R03_FINE Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
