...
!      mmmd=float(mm+1)*dt/3600./24.                       !mmmd is 
      ...
      Dzzh = sndzh*min(abs(Dzh),oscilflag)
      if ((abs(dzz) == oscilflag) .or. (abs(dzzh) ==oscilflag)) then
          write(*,*) 'Comp=',j,'timestep=',mm,'dz=',dz,'dzh=',dzh
          write(*,*) 'Qsum=',Qsum,'Qmarsh=',Qmarsh(j,2)
          write(*,*) 'OWstg(t-1)=',Es(j,1),'OWstg(t)=',Es(j,2)
          write(*,*) 'Mstg(t-1)=',Eh(j,1),'Mstg(t)=',Eh(j,2)
          write(1,*) 'Comp=',j,'timestep=',mm,'dz=',dz,'dzh=',dzh
