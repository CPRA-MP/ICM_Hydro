module common_array_R
	...
	integer, allocatable :: ncomp_R(:), ndt_R(:), ioutf_R(:), nlat_R(:), nlat_R_ori(:), n_R(:), n_SAL_R(:), n_TMP_R(:), n_FINE_R(:), n_SAND_R(:)
	...
	real :: days_all
	integer, allocatable :: latFlowLoc_R(:,:), latFlowType_R(:,:),latFlowXsec_R(:,:)
! SAL TMP, FINE, SAND
...
! R00
		...
		allocate(latFlowLoc_R(num_reg, num_lat))
		latFlowLoc_R=0
		allocate(latFlowType_R(num_reg, num_lat))
		latFlowType_R=0
		allocate(latFlowXsec_R(num_reg, num_lat))
		latFlowXsec_R=0
