...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_FINE_R08(npr, ioutfile, input_file, rday, ndtr, nlatr_ori, nlatr, latFlowLoc, latFlowTp, latFlowXsec)
    use precision
    !use params
    use mod_arrays_FINE_R08
    
    use sed_properties_FINE_R08
    use spline_func
    ...
    if (ioerr /= 0) then
        write(*, *) 'ERROR reading the input file: fine.inp'
        stop
    endif
    
    !read(LUN, '(a)') RefDate   ! Reference date, Format: MM-DD-YYYY_HH:MM:SS or MM-DD-YYYYTHH:MM:SS (not used at the moment)
    read(LUN, *) fine_init   ! Reference date, Format: MM-DD-YYYY_HH:MM:SS or MM-DD-YYYYTHH:MM:SS (not used at the moment)
...
end subroutine init_FINE_R08


subroutine cal_FINE_R08(n, npr, ioutfile, nlatt, inp_depth, inp_area, inp_flow, out_fine, Q_lat_from_ICM, FINE_lat_from_ICM, FINE_upstream_from_ICM, FINE_terminal_from_ICM)
    use precision
    !use params
    use mod_arrays_FINE_R08
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R08_FINE Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
  ...
  ! subsequently into the C(:,:,:) array (see above)
    if (n /= nt) then
        do k = 1, np            ! loop over sediment partical classes
            do i = 2, nx - 1    ! loop over cross-sections
                ...
                do j=1, Nlat
                    ...
                    if (i >= ilat .AND. i < (ilat+jlat))then
                        ...
                        !coupling from ICM
                        if (Q_lat_from_ICM(j) .gt. 0. .and. FINE_lat_from_ICM(j) .ge. 0. .and. klat .eq. 3)then
                            SrcTerm = SrcTerm + Q_lat_from_ICM(j)/real(jlat)*(FINE_lat_from_ICM(j)-CN(i,k))/(abs(flow(i))+Q_lat_from_ICM(j)/real(jlat))
                        endif
                        
                        if (klat.eq.2) then
                            print*, 'Rating curves for lateral FINE input have not been supported yet.'
                            stop
                        endif
                    endif
                enddo
            ...
            if(FINE_upstream_from_ICM .ge. 0. )CN1(1, k)=FINE_upstream_from_ICM   !coupling from ICM
            if (flow(i) < 0.)then
                CN1(1, k)=CN1(2, k)
            endif
            ...
            if (flow(i) > 0. )then
                ...
                if(FINE_terminal_from_ICM .ge. 0. )CN1(nx, k)=FINE_terminal_from_ICM  !coupling from ICM
            endif
