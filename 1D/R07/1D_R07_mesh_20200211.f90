...
!    close(51)
!	close(61)
!end subroutine end_R07

subroutine init_R07(npr, ifile, input_file, rday, ndt, nlat_ori, nlat, latFlowLoc, latFlowTp, latFlowXsec, outa, outb, outc, outd, wl_lat, Q_terminal)
    use constants_module_R07
    use arrays_module_R07
    use arrays_section_module_R07
    use var_module_R07
    use matrix_module_R07
    use sgate_module_R07
...
!	endif
    ...
    do i=1,ncomp
        read(999, *) x, sk(i)
        call readXsection_R07(i,(1.0/sk(i)),timesDepth)
...
end subroutine init_R07



subroutine cal_R07(n, npr, ifile, nlat, outa, outb, outc, outd, wl_lat, WL_terminal_from_ICM, Q_upstream_from_ICM, Q_lat_from_ICM, Q_terminal)
...
!	pause 0
	...
	t=n*dtini/60.    ! t is in min, dtini is in sec
        ...
        do i=1,noLatFlow
			...
			!		print*,'R07 ERROR: Q_lat_from_ICM at #',i
			!		stop
			!	endif
...
!-------------------------------------
		...
		if(mod(n+1,24*saveFrequency) .eq. 0 .or. (n.eq.0))write(*,*)'R07: Nstep =', n+1, 'Days = ' &
