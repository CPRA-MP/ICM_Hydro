...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_SAND_R05(npr, ioutfile, input_file, rday, ndtr, nlatr_ori, nlatr, latFlowLoc, latFlowTp, latFlowXsec)
    use precision
    !use params
    use mod_arrays_SAND_R05
    
    use sed_properties_SAND_R05
    use spline_func
...
end subroutine init_SAND_R05


subroutine cal_SAND_R05(n, npr, ioutfile, nlatt, inp_depth, inp_area, inp_flow, out_sand, Q_lat_from_ICM, SAND_lat_from_ICM, SAND_upstream_from_ICM, SAND_terminal_from_ICM)
    use precision
    !use params
    use mod_arrays_SAND_R05
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R05_SAND Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
