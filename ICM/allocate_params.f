...
!output summary arrays for 500 m grid !-EDW     
      ...
      allocate(stage_var_summer_500m(n_500m_cells))
...
!      allocate(wspd(simdays))
      ...
      allocate(TideData(tidesteps+24,tidegages))  ! need to be revised to account for tide transpose
      allocate(transposed_tide(tidegages,2))
      allocate(weighted_tide(Mds-tidegages,5))
      allocate(surge(tidesteps+24,Mds)) !-EDW ! need to be revised to account for tide transpose
