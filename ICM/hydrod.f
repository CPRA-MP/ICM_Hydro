...
!>> Assume Boundary condition sediment is evenly distributed amongst clay and silt size classes and half of the clay is flocced.
      ...
      do jjk=1,mds
          jj=KBC(jjk)
		do kk=1,4
              Css(jj,2,kk)=BCTSS(jj)*(1.+ 0.5*sin(pi*wd(kday)/180.))
     &                        *BCSedRatio(kk)         !BCSedRatio(k) is multipler on BCTss that separates into different classes
          enddo
...
!>> 1D-2D coupling code start
	if (nlc>0) then
          do i=1,nlc
              Es(lcf2D(i),2) =  lcH(i) ! yw assign stage from 1D to the fictional compartments. need to be revise for miltiple leteral flow condition
     ...
     &                -Latr1(i))+Latr4(i))
		...
		!>> Calculate hydraulic radius from average flow depth (if open channel)
          ...
          !>> Determine direction of flow
                  ...
                  !    write(*,*) 'jus(i) = ',jus(i)
                  !    write(*,*) 'jds(i) = ',jds(i)
                  !    write(*,*) 'Es(jus(i),2) = ',Es(jus(i),2)
                  !    write(*,*) 'Es(jds(i),2) = ',Es(jds(i),2)
                  !    write(*,*) 'Q_filter1  = ',Q_filter1 
                  !    write(*,*) 'Q_filter2  = ',Q_filter2
                  !    write(*,*) 'Q(i,2)  = ',Q(i,2)
                  !    pause
                  !endif   
          ...
          !>> update upwind factor for salinity/WQ dispersion if channel velocity is greater than threshold value
              ...
              !if (i == 41) then
              !    write(*,*) 'Q =', sqrt(abs(Deta))*Resist*sn*dkd
              !    write(*,*) 'Q_filter1=', Q_filter1
              !    write(*,*) 'Q_filter2=', Q_filter2
              !    write(*,*) 'Q(i,2)=', Q(i,2)
              !    write(*,*) 'Deta=', Deta
     ...
     & but this should be fixed in the input file.'
          ...
          !>> Set zero multiplier if target link discharge is lower then the threshold discharge       
              elseif (Latr9(i) == 7) then
                  ...
                  !write(*,*) 'i = ',i
                  !write(*,*) 'Latr2(i) = ',Latr2(i)
                  !write(*,*) 'Q(Latr2(i) = ',Q(Latr2(i))
                  !write(*,*) 'Latr10(i) = ',Latr10(i)
              endif
...
!>> If calculated flowrate is greater than needed to exchange all available water, set flowrate to the max flowrate calculated in celldQ
	...
	enddo												!JAM Oct 2010
      ...
      !WRITE(501,1111) (ES(jj,2),jj=1,N)  ! for debug only yw
      !WRITE(502,1112) (Q(jj,2),jj=1,M)  ! for debug only yw
      !WRITE(503,1111) (S(jj,2),jj=1,N)  ! for debug only yw
      !pause  
