...
!>> debug files      
      open(unit=501,file='debug_WL.dat',form='formatted')                                                     ! yw debug
      open(unit=502,file='debug_FLO.dat',form='formatted')                                                     ! yw debug  
      open(unit=503,file='debug_SAL.dat',form='formatted')                                                     ! yw debug 
      
!>> 1d2d coupling setup files
      ...
      if (nlc>0) then      
        ...
        enddo
	    do i = 1,nlc
		    read(403,*) lcf2D(i)   ! read the list of lateral flow fictional ICM compartment
...
!	  time step of MESH n_1d=1
	  ...
	  !!! NTs is the no of time steps in ICM while ntim is the no of time steps in MESH. THESE two needs to be equal

      do  mm= 1, NTs						! ******do loop ends at ~ line 438
...
!>> Saving lateral flow           
          if (nlc>0) then
		      do i = 1,nlc                          ! loop over lateral flow compartment
				  lcQ(i) = -1.0*Q(lcl2D(i),2) ! yw need to revise for multiple lateral flow
                  !write(*,*) 'lcl2D(i) = ',lcl2D(i)
