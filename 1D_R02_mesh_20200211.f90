...
!    close(51)
!	close(61)
!end subroutine end_R02

subroutine init_R02(npr, ifile, input_file, rday, ndt, nlat_ori, nlat, latFlowLoc, latFlowTp, latFlowXsec, outa, outb, outc, outd, wl_lat, Q_terminal)
    use constants_module_R02
    use arrays_module_R02
    use arrays_section_module_R02
    use var_module_R02
    use matrix_module_R02
    use sgate_module_R02
    use xsec_attribute_module_R02
...
!	endif
    ...
    do i=1,ncomp
        read(999, *) x, sk(i)
        call readXsection_R02(i,(1.0/sk(i)),timesDepth)
...
end subroutine init_R02



subroutine cal_R02(n, npr, ifile, nlat, outa, outb, outc, outd, wl_lat, WL_terminal_from_ICM, Q_upstream_from_ICM, Q_lat_from_ICM, Q_terminal)
        ...
        do i=1,noLatFlow
			...
			elseif (latFlowType(i) .eq. 3) then
				if(Q_lat_from_ICM(i)<-99990.)then
					print*,'R02 ERROR: Q_lat_from_ICM at #',i
					stop
				endif
...
!-------------------------------------
		...
		if(mod(n+1,24*saveFrequency) .eq. 0 .or. (n.eq.0))write(*,*)'R02: Nstep =', n+1, 'Days = ' &
