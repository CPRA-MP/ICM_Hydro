...
!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!
subroutine init_SAL_R03(npr, ioutfile, input_file, rday, ndtr, nlatr_ori, nlatr, latFlowLoc, latFlowTp, latFlowXsec)
  use precision
!  use params
  use mod_arrays_SAL_R03

  use sed_properties_SAL_R03
  use spline_func
...
!  use params
  ...
  !integer :: LUN, ioerr
    ...
    if(n.eq.1 .or. modulo(time(n+1), DtUser*24).eq.0. .or. n.eq.(nt-1))write(*,*),'R03_SAL Nstep =', n, 'Days = ', int(time(n+1)/3600./24.), real(n)/real(nt-1)*100., '% completed'
